  ...
  readFile(filename, manufacturer) {
    ...
    .then(response => {
      ...
      for (let j = 0; j < cars.length; j++) {
        let theCar = cars[j];
        if (!model) {
          model = new Model(this.firstTwoWords(theCar.variant), manufacturer);
        } else {
          if (theCar.variant !== model.name) {
            const words = this.commonWords(model.name, theCar.variant);
            if (words.length > 0) {
    ...
    let cars = ['id; variant; power; real_weight; official_weight; options; start_date; model_id']
      .concat(this.state.content.flatMap(manufacturer =>
        manufacturer.models.flatMap(model =>
          model.cars.map(car =>
            `${car.id};${this.csvEscape(car.variant)};${car.power};${car.realWeight};${car.officialWeight};${this.csvEscape(car.options)};${car.startDate};${car.model.id}`
          )
        )
      ));
